@extends('layouts.app');
@section('content')

    <!-- Banner -->

    <section class="banner" style="height: 250px">
        <picture>
            <img src="https://shpilkin.ru/wp-content/uploads/2018/06/banner-1190x320.jpg" alt="реклама"
                 style="width: 100%; height: 100%; object-fit: fill;">
        </picture>
    </section>

    <!-- /Banner -->

    <!-- Название кухни -->
    <section>
        <h1 class="title" style="font-weight: normal">
            Корейская кухня
            <i id="mobile-arrow"></i>
        </h1>
    </section>
    <!-- /Название кухни -->


    <!-- То что будет в мейне -->
    <section class="catalog-section">

        <!-- Рядом то что стойт ктегорий -->
        <div class="sidebar">
            <div class="sidebar-stick" style="">
                <div class="sidebar-title">Категории</div>
                <ul class="sidebar-categories">
                    <li>
                        <a class="" href="/">
                            Все кафе и рестораны
                        </a>
                    </li>
                    <li>
                        <a class="" href="{{route('category',4)}}">
                            Бары
                        </a>
                    </li>
                    <li>
                        <a class="" href="{{route('category',1)}}">
                            Рестораны
                        </a>
                    </li>
                    <li>
                        <a class="" href="{{route('category',3)}}">
                            Кофейня
                        </a>
                    </li>
                    <li>
                        <a class="" href="{{route('category',2)}}">
                            Кафе
                        </a>
                    </li>
                    <li>
                        <a class="" href="{{route('category',5)}}">
                            Антикафе
                        </a>
                    </li>
                    <li>
                        <a class="" href="{{route('category',6)}}">
                            Клуб
                        </a>
                    </li>
                    <li>
                        <a class="" href="{{route('category',7)}}">
                            Лаундж
                        </a>
                    </li>
                    <li>
                        <a class="" href="{{route('institutions.type',2)}}">
                            Китайская кухня
                        </a>
                    </li>
                    <li>
                        <a class="" href="{{route('institutions.type',1)}}">
                            Русская кухня
                        </a>
                    </li>
                    <li>
                        <a class="" href="{{route('institutions.type',3)}}">
                            Казахская кухня
                        </a>
                    </li>
                </ul>
                <div class="sidebar-information">
                    <div class="sidebar-information--title" style="font-weight: normal;">ВМЕСТО ЗВОНКОВ -
                        СООБЩЕНИЯ.
                    </div>
                    <div class="sidebar-information--description">
                        <p>Иногда кажется, что созвониться быстрее, верно? Но это отнюдь не всегда так. В бесконечных
                            диалогах мы тратим кучу времени. При этом - редко удается делать что-то параллельно. А
                            сообщения - это сжатый кусок конкретной информации. Это КОНКРЕТИКА. Сегодня есть не только
                            текст, но и аудио. Его можно написать в любое время, в любом месте и... продолжить делать
                            свои дела. Просто попробуйте делать это хотя бы пару дней.</p>
                    </div>
                    <h3>Здесь был Арыстанбек</h3>
                    <div class="sidebar-information--description">
                        <p>Листая меню Вы сможете найти блюда разной остроты, подходящие для простого перекуса и
                            праздничного застолья. От кимчи до курицы по-сеульски - всё найдется на Namba Food!
                            Для
                            Вашего удобства мы собрали все корейские кафе и рестораны Бишкека на одном сервисе.
                            Выбирайте то, что желаете попробовать и профессиональные повара приготовят настоящую
                            корейскую еду, а курьеры доставят её в офис или к Вам домой меньше чем за час после
                            утверждения заказа.</p>
                        <p>Несмотря на то, что корейская кухня очень острая - тонкий баланс вкусовых сочетаний
                            присутствует в ней, что делает её еще более необычной и вкусной!</p>
                    </div>
                </div>
            </div>
        </div>
        <!-- /Рядом то что стойт ктегорий -->


        <!-- cards -->
        <div class="catalog-wrap">

            <!-- card -->
            @foreach($institutions as $institution)
                <a href="{{route('institutions.show',['institution'=>$institution])}}" class="cafe-item " data-id="1022"
                   data-filter=".all  .new ">
                    <div class="cafe-item--thumb">
                        @if($institution->picture)
                            <img src="{{asset('/storage/' . $institution->picture)}}"
                                 alt="">
                        @else
                            <img
                                src="data:image/png;base64,"
                                alt="Нету картинки">
                        @endif
                        <div class="cafe-item--rating">
                            <div class="information--item information--work">
                                <div class="information__description">
                                    <div class="float-left">

                                        <div class="raiting-container
        rating-result one-star-rating one-star-rating__2
">
                                            <label for="rating_5_692899226" class="hover-area hover-area__5"></label>
                                            <input id="rating_5_692899226" class="hide rating-input rating_5"
                                                   type="radio" name="rating" value="5">
                                            <label for="rating_4_692899226" class="hover-area hover-area__4"></label>
                                            <input id="rating_4_692899226" class="hide rating-input rating_4"
                                                   type="radio" name="rating" value="4">
                                            <label for="rating_3_692899226" class="hover-area hover-area__3"></label>
                                            <input id="rating_3_692899226" class="hide rating-input rating_3"
                                                   type="radio" name="rating" value="3">
                                            <label for="rating_2_692899226" class="hover-area hover-area__2"></label>
                                            <input id="rating_2_692899226" class="hide rating-input rating_2"
                                                   type="radio" name="rating" value="2">
                                            <label for="rating_1_692899226" class="hover-area hover-area__1"></label>
                                            <input id="rating_1_692899226" class="hide rating-input rating_1"
                                                   type="radio" name="rating" value="1">
                                            <div class="stars-container"></div>
                                            <div class="rating-value rating-value__0">0</div>
                                            <div class="rating-value rating-value__1">1</div>
                                            <div class="rating-value rating-value__2">2</div>
                                            <div class="rating-value rating-value__3">3</div>
                                            <div class="rating-value rating-value__4">4</div>
                                            <div class="rating-value rating-value__5">5</div>
                                        </div>
                                    </div>
                                    <span class="align-top">
                                        5.0
                                    </span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="cafe-item--wrap">
                        <div class="cafe-item--title">
                            <div class="cafe--name">
                                {{$institution->category->name}} «{{$institution->name}}»
                            </div>
                        </div>
                        <ul class="cafe-item--categories">
                            @foreach($institution->dishes()->with('kitchen')->get()->pluck('kitchen.name')->unique() as $type)
                                <div class="categories__item">{{$type}}</div>
                            @endforeach
                        </ul>
                        <div class="cafe-item--information">
                            <div class="information--item information--check">
                                <div class="information__title">Средний чек:</div>
                                <div class="information__description">{{$institution->averageCheck()}} сом</div>
                            </div>
                            <div class="information--item information--delivery">
                                <div class="information__title">Доставка:</div>
                                <div class="information__description">
                                    {{$institution->delivery}} сом
                                </div>
                            </div>
                            <div class="information--item information--work">
                                <div class="information__title">Время работы:</div>
                                <div class="information__description">

                                    11:00 – 00:00

                                    <i>
                                        Без выходных
                                    </i>
                                </div>
                            </div>
                            <div class="information--item information--rating">
                                <div class="information__title">Рейтинг:</div>
                                <div class="information__description">
                                    <div class="float-left">

                                        <div class="raiting-container
        rating-result one-star-rating one-star-rating__2
">
                                            <label for="rating_5_918430223" class="hover-area hover-area__5"></label>
                                            <input id="rating_5_918430223" class="hide rating-input rating_5"
                                                   type="radio" name="rating" value="5">
                                            <label for="rating_4_918430223" class="hover-area hover-area__4"></label>
                                            <input id="rating_4_918430223" class="hide rating-input rating_4"
                                                   type="radio" name="rating" value="4">
                                            <label for="rating_3_918430223" class="hover-area hover-area__3"></label>
                                            <input id="rating_3_918430223" class="hide rating-input rating_3"
                                                   type="radio" name="rating" value="3">
                                            <label for="rating_2_918430223" class="hover-area hover-area__2"></label>
                                            <input id="rating_2_918430223" class="hide rating-input rating_2"
                                                   type="radio" name="rating" value="2">
                                            <label for="rating_1_918430223" class="hover-area hover-area__1"></label>
                                            <input id="rating_1_918430223" class="hide rating-input rating_1"
                                                   type="radio" name="rating" value="1">
                                            <div class="stars-container"></div>
                                            <div class="rating-value rating-value__0">0</div>
                                            <div class="rating-value rating-value__1">1</div>
                                            <div class="rating-value rating-value__2">2</div>
                                            <div class="rating-value rating-value__3">3</div>
                                            <div class="rating-value rating-value__4">4</div>
                                            <div class="rating-value rating-value__5">5</div>
                                        </div>
                                    </div>
                                    <span class="align-top">
                                        5.0
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div class="information--sales">
                        </div>
                    </div>
                </a>
            @endforeach

            <!-- /card -->

            <div class="d-flex justify-content-center mt-5">
                {{ $institutions->links('vendor.pagination.bootstrap-4') }}
            </div>
        </div>

        <!-- /cards -->


    </section>
    <!-- /То что будет в мейне -->

@endsection
